        ...
        for(prf_idx = 0; prf_idx < BLE_NB_PROFILES ; prf_idx++)
        {
            ...
            switch (init_type)
            {
                case RWIP_RST:
                {
                    ...
                    // PARASOFT FIX: Add bounds check for prf_itf array access
                    // Original violation: "Possibly accessing array prf_itf out of bounds"
                    // Root cause: prf_id is uint8_t (0-255) but prf_itf[PRF_ID_MAX] has limited size
                    // Solution: Add same bounds check used in prf_env_get() function (line 1103)
                         ...
                         uint16_t* p_start_hdl)
{
    ...
    // PARASOFT FIX: Add bounds check for prf_itf array access  
    // Same issue as line 907 - add PRF_ID_MAX bounds check
    if((prf_id != PRF_ID_INVALID) && (prf_id < PRF_ID_MAX))
    {
