...
typedef  struct
{
    ...
    uint8_t flag_nack;       // check slave:1:nack
}  I2C_MSG;
...
void i2c_init(uint32_t slaveAddr, uint32_t baudRate)
{
    ...
    gpio_scfun_sel(0x02, 0);
    gpio_scfun_sel(0x03, 0);
...
#define SLV_ADDR 0x50<<1//0xae

#define BUF_LEN   16
uint8_t i2c_data_org=0x40;
void i2c_test_e2prom(void)
{

    uint8_t buf[BUF_LEN + 5] = {0};

    i2c_init(SLV_ADDR>>1,I2C_BAUD_100KHZ);
    uart_printf("**************I2C_START **************\r\n");
    ...
    if(OK == i2c_read(SLV_ADDR>>1,0,buf,BUF_LEN))
    {
        for(int i = 0; i < BUF_LEN;i++)
        {
    ...
    Delay_ms(100);
     uart_printf("\r\nwrite data =%x,",i2c_data_org);
    for(int i = 0; i < BUF_LEN;i++)
    {
        buf[i] =i2c_data_org+ i;
        if(OK == i2c_write(SLV_ADDR>>1,i,&buf[i],1))
        {
            uart_printf("i2c_write ok\r\n");
        } Delay_ms(100);
    }
    i2c_data_org += 0x10;
    Delay_ms(100);
    uart_printf("after write read data:");
    for(int i = 0; i < BUF_LEN;i++)
    {
        buf[i] = 0;
    }
...
void i2c_test(void)
{
    uart_printf("==============I2C_TEST ==============\r\n");
    uart_printf("==============change to uart0 print ...........==============\r\n");
    uart_printf("==============send any data to exit i2c test==============\r\n");
    Delay_ms(1000);
    while(1)
    {
        uart0_init(1000000);
        ...
        {
            i2cc_stop();
            uart_printf("exit i2c test\r\n ");
            Delay_ms(100);
            uart_printf("==============exit i2c test==============\r\n");
            uart_printf("==============change to uart1 print ...........==============\r\n");
            clear_uart1_buffer();
            clear_uart_buffer();
            return;
        }
