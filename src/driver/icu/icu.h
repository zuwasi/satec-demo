...
enum sleep_type
{
    ...
    MCU_LOW_POWER_SLEEP = 2, //cpu idle,16M,PLL STOP,reduce cpu voltage
...
void rw_ip_sleep_test(uint32 time);
uint8_t get_rw_ip_sleep_test_flag(void);
void int_init(void);
void rw_sleep(void);
void cpu_low_power_sleep(void);
void cpu_idle_sleep(void);
void pll_test(void);
void mcu_buck_power(void);
...
// mcu clock power down has bug,so used this
#if 0
__INLINE void WFI0(void)
{
    uint32_t __l_rd;
#pragma arm
    __asm
